...
async function handleAddProduct(req: NextRequest, { user }: { user?: any }) {
  try {
    ...
    // Prepare variants for sequential insertion
    ...
    console.log("Inserting variants with sequential serial numbers:", variantsToInsert.length);
    
    try {
      const result = await insertVariantsWithUniqueSerials(variantsToInsert, user.id, authenticatedSupabase);
      
      if (!result.success) {
        console.error("Error creating variants:", result.error);
        return NextResponse.json({
          success: false,
      ...
      console.error("Unexpected error creating variants:", error);
      return NextResponse.json({
        success: false,
        error: "Error creating variants",
